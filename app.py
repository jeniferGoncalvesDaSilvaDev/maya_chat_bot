...
if "historico_interacao" not in st.session_state:
    st.session_state.historico_interacao = []
...
def analisar_sentimento_responder(texto_usuario):
    ...
    st.session_state.historico_interacao.append({"texto": texto_usuario, "sentimento": sentimento, "score": score})
...
def encontrar_resposta_semelhante(nova_entrada):
    if not st.session_state.historico_interacao:
        return None
    ...
    embeddings_historico = [modelo_embeddings.encode(entry["texto"], convert_to_tensor=True) for entry in st.session_state.historico_interacao]
    ...
    if similaridades[indice_mais_similar] > 0.7:
        return st.session_state.historico_interacao[indice_mais_similar]["texto"]
...
if st.button("Sair"):
    ...
    st.session_state.historico_interacao = []  # Limpa o histórico ao sair
...
if texto_usuario:
    ...
    if len(st.session_state.historico_interacao) > 0:
        st.subheader("Histórico de Interações:")
        for entry in st.session_state.historico_interacao:
